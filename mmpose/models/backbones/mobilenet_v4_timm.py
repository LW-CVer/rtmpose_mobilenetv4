...
@register_model
def mobilenetv4_conv_small(pretrained: bool = False,num_classes=1000, **kwargs) -> MobileNetV3:
    """ MobileNet V4 """
    model = _gen_mobilenet_v4('mobilenetv4_conv_small', 1.0, pretrained=pretrained, num_classes=num_classes,**kwargs)
    return model


@register_model
def mobilenetv4_conv_medium(pretrained: bool = False, num_classes=1000,**kwargs) -> MobileNetV3:
    """ MobileNet V4 """
    model = _gen_mobilenet_v4('mobilenetv4_conv_medium', 1.0, pretrained=pretrained, num_classes=num_classes,**kwargs)
    return model
...
if __name__ == '__main__':
    ...
    # model=mobilenetv4_conv_medium(pretrained= False, num_classes=1000,features_only=True)
    model=_gen_mobilenet_v4("mobilenetv4_conv_medium",1.0,features_only=True)
    # model.load_state_dict(torch.load("mobilenetv4_conv_small.e1200_r224_in1k/pytorch_model.bin"))
    model.eval()

    x=torch.rand(1,3,256,192)

    out=model(x)
    # out=out.softmax(dim=1) * 100
    # print(torch.max(out),torch.argmax(out))
